...
| **特征真实性** | 结构化模态完全由真实数据驱动：推理侧 20 项结构化特征**真实字段优先**（HR 采集），缺失回退训练分布中位/众数常量，无随机注入（0002 迁移）。⚠️ **行为时序模态当前版本为 demo 数据**（由 employee.id 播种确定性生成，见 ml/feature_provider.py），风险 API 响应以 `behavior_data_source: "demo"` 标注；接入真实行为日志为路线图项 |
