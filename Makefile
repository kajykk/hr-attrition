.PHONY: help dev dev-api up down build start stop logs ps migrate migrate-gen test backend-test frontend-install frontend-dev frontend-build lint clean

# HRA 项目便捷命令（D10 3.2）

help:  ## 显示帮助
	@echo "HRA 项目命令："
	@echo "  make dev              启动本地开发（后端 API 前台运行）"
	@echo "  make dev-api          前台启动后端 API（uvicorn --reload :8000）"
	@echo "  make frontend-dev     启动前端开发服务器（vite :5173，另开终端）"
...
dev: dev-api  ## 启动本地开发（后端 API 前台；前端另开终端跑 make frontend-dev）

dev-api:  ## 前台启动后端 API 开发服务器（uvicorn --reload，端口 8000）
